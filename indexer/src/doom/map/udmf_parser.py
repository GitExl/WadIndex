...
from doom.map.map import Line, LineFlags, Sector, Side, Thing, ThingFlags, Vertex
    ...
    def __init__(self):
        ...
        self.lexer: Lexer = Lexer(
            [
                ...
                Rule(UDMFToken.FLOAT, '[+-]?[0-9]+\\.[0-9]*(?:[eE][+-]?[0-9]+)?', process=lexer.process_float),
                Rule(UDMFToken.INTEGER, '[+-]?[1-9]\\d*|0', process=lexer.process_int),
                Rule(UDMFToken.INTEGER, '0[0-9]+', process=lexer.process_int_base8),
                Rule(UDMFToken.INTEGER, '0x[0-9A-Fa-f]+', process=lexer.process_int_base16),
                Rule(UDMFToken.KEYWORD, '[^{}();"\'\n\t ]+'),
                Rule(UDMFToken.STRING, '"(?:[^"\\\\]|\\\\.)*"', process=lexer.process_string),
            ]
        )
    ...
    def parse(self, text: str):
        ...
        while True:
            ...
            if token is None:
                break
            if token[0] != UDMFToken.IDENTIFIER:
                raise UDMFParserError('Expected an identifier, got "{}".'.format(token[1]), expand_token_position(token))
            ...
            if identifier == 'namespace':
                ...
                raise UDMFParserError('Unknown root identifier "{}".'.format(identifier), expand_token_position(token))
